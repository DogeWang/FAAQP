...
class SPNEnsemble:
    ...
    def _cardinality_greedy(self, query, rdc_spn_selection=False, rdc_attribute_dict=None, dry_run=False,
        ...
        # print(first_spn, next_mergeable_relationships, next_mergeable_tables)
    ...
    def _evaluate_group_by_spn_ensembles(self, query):
        ...
        for spn, attribute_list in dict_items:
            for attribute in attribute_list:
                ...
                attribute_counter += 1
        # print('group_by_permutation:', group_by_permutation)
        result_tuples = None
        ...
        for spn, attribute_list in dict_items:
            ...
            group_bys_scope, temporary_results, temporary_results_translated = spn.evaluate_group_by_combinations(
                attribute_list,
                conditions)
            if not temporary_results_translated:
                return group_bys_scope, temporary_results, temporary_results_translated
            ...
            if result_tuples is None:
                ...
                # 结果维度大小事相乘的，但是什么时候会出现这种情况？两边连接分别有GROUP BY语句？这么做结果准确性能保证？
        ...
        # print('Results:', group_bys_scopes, result_tuples, len(result_tuples_translated))
    ...
    def evaluate_query(self, query, rdc_spn_selection=False, pairwise_rdc_path=None,
        ...
        if len(query.group_bys) > 0:
            ...
            if not result_tuples_translated:
                technical_group_by_scopes = None
                logger.debug(f"\t\tNone Group in SPN!")
                return None, None
            else:
        ...
        if query.query_type == QueryType.CARDINALITY or any(
            ...
            # print('spn_ensemble.py 772 artificially_added_conditions:', artificially_added_conditions)
            ...
            print('factors, factor_values:', factors, factor_values)
            if debug:
                if len(query.group_bys) == 0:
        ...
        if all_operations_of_type(AggregationType.SUM, query) or all_operations_of_type(AggregationType.AVG, query):
            ...
            for aggregation_operation_type, aggregation_type, factors in query.aggregation_operations:
                ...
                # which aggregation
                    ...
                    if not isinstance(aggregation_result, float) or aggregation_result:
                        logger.debug(
    ...
    def _cardinality_with_injected_start(self, query, first_spn, next_mergeable_relationships, next_mergeable_tables,
        ...
        # print('spn_ensemble.py 1048 auxilary_query.table_where_condition_dict:', auxilary_query.table_where_condition_dict)
        ...
        while len(query.relationship_set) > 0:
            ...
            # print('spn_ensemble.py 1086', next_spn)
        ...
        # print('spn_ensemble.py 1211:', dry_run, factors, self.cached_expecation_vals, gen_code_stats)
        values, cardinality, formula = evaluate_factors(dry_run, factors, self.cached_expecation_vals,
                                                        gen_code_stats=gen_code_stats)
        print(values, cardinality, formula)

...
def evaluate_aqp_queries(ensemble_location, query_filename, target_path, schema, ground_truth_path,
    ...
    for query_no, query_str in enumerate(queries):
        ...
        try:
            ...
            if ground_truth is not None and ground_truth[query_no]:
                true_result = ground_truth[query_no]
                if not aqp_result:
                    true_result = true_result[0][0]
                    if not true_result:
                        logger.info(f"\t\tSPN success!")
                        logger.info(f"\t\t{'total_bins: ':<32}{0:.2f}%")
                        logger.info(f"\t\t{'average_relative_error: ':<32}{0:.2f}%")
                        logger.info(f"\t\t{'false_bin_percentage: ':<32}{0:.2f}%")
                        ...
                        logger.info(f"\t\tSPN miss all results!")
                        logger.info(f"\t\t{'total_bins: ':<32}{100:.2f}%")
                        logger.info(f"\t\t{'average_relative_error: ':<32}{100:.2f}%")
                        logger.info(f"\t\t{'false_bin_percentage: ':<32}{100:.2f}%")
                        csv_rows.append({'approach': ApproachType.MODEL_BASED,
                                         'query_no': query_no,
                                         'latency': latency,
                                         # 'average_relative_error': true_result,
                ...
                if isinstance(aqp_result, list):
                    ...
                    true_result = true_result[0][0]
                    if not true_result and true_result != 0:
                        logger.info(f"\t\t{'SPN fail!':<32}{true_result}")
                        csv_rows.append({'approach': ApproachType.MODEL_BASED,
                                         'query_no': query_no,
                                         'latency': latency,
                                         'average_relative_error': -3,
                                         'bin_completeness': 0,
                                         'total_bins': 0,
                                         'query': query_str,
                                         'sample_percentage': 100
                    ...
                    if true_result == 0 and aqp_result != 0:
                        logger.info(f"\t\t{'SPN fail!':<32}{true_result}")
                        csv_rows.append({'approach': ApproachType.MODEL_BASED,
                                         'query_no': query_no,
                                         'latency': latency,
                                         'average_relative_error': 1,
                                         'bin_completeness': -2,
                                         'total_bins': -2,
                                         'query': query_str,
                                         'sample_percentage': 100
                        ...
                        continue
                    elif true_result == 0 and aqp_result == 0:
                        csv_rows.append({'approach': ApproachType.MODEL_BASED,
                                         'query_no': query_no,
                                         'latency': latency,
                                         'average_relative_error': 0,
